...
def check_init_ssh():
    ...
    if not os.path.exists(ssh_dir):
        ...
        if exit_code:
            raise ValueError(f"SSH - Create ~/.ssh errored {exit_code}")


def add_ssh_key(env_key: str):
    key = os.getenv(env_key)
    if not key:
        logging.error(f"Add key {env_key} is not set in environment")
        return
    if key[-1] != "\n":
        key = key + "\n"
    check_init_ssh()
    ...
    if exit_code:
        raise ValueError(f"SSH - Add key errored {exit_code}")
...
def add_ssh_host(host: str):
    ...
    if exit_code:
        ...
        with open(known_hosts, "a+") as write_hosts:
            ...
            if exit_code:
                raise ValueError(f"Repo SSH - Add host errored {exit_code}")


def clear_ssh_keys():
    args = ["ssh-add", "-D"]
    logger.info(f"Clear all keys")
    exit_code = logged_subprocess("Clear keys", ["ssh-add", "-D"])
    if exit_code:
        raise ValueError(f"SSH - Clear all keys errored {exit_code}")
