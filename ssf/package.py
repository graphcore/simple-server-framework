...
def package(ssf_config: SSFConfig):
    ...
    def package_decl(
        ...
        src_dir, root_src_dir, files = build_file_list(
            src_dir, include, exclude, always
        )
    ...
    requirements_path = os.path.join(package_dir, "ssf_package_requirements.txt")
    requirements_file = open(requirements_path, "w+")
    requirements_file.write("-r src/ssf/requirements.txt\n")
    ...
    if app_dir:
        ...
        app_requirements = ssf_config.application.dependencies["python"]
        ...
        with open(run_script, "w") as f:
            ...
            f.write(
                f"python -m ssf.cli --config {app_config_package_path} run $SSF_OPTIONS\n"
            )
    ...
    build_command = "docker --debug --log-level debug build --progress plain"
    ...
    if exit_code:
        logger.error(f"Tarball packing errored {exit_code}")
    ...
    with temporary_cwd(package_dir):
        ...
        if exit_code:
            logger.error(f"Container image build {package_tag} errored {exit_code}")
            raise ValueError(
                f"Failed to build container image {package_tag} {exit_code}"
            )
